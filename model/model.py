...
web3 = Web3(KeepAliveRPCProvider(host='ohio.cs.nccu.edu.tw', port='80'))
...
class Organization:
	def __init__(self, address):
		...
		self.entity = web3.eth.contract(abi, code=code, address=address)
	...
	def find_account(self, user):
		if user is None:
			user = defaultUser
		addr = self.entity.call().account_map(user.address)
		if addr == zero:
			return None
	...
	def account_list(self):
		return self.entity.call().list_all_account()
...
class Account:
	...
	def papers_list(self):
		return self.read_data().list_all_papers()
	def invite_review(self, reviewer, paper, value):
		token1 = self.write_data(value).invite_review(reviewer, paper)
		send_transaction(token1,'make invite')
		i = self.invites_list()[-1]
		token2 = self.write_data().notice_reviewer(i)
		send_transaction(token2,'connect to user')
		return i
	def cancel_invite(self, i):
		token = self.write_data().cancel_invite(i)
		send_transaction(token,'cancel invite')
	def invites_list(self):
		return self.read_data().list_all_invites()
	def request_list(self):
		return self.read_data().list_all_requests()
	def done_review(self, inv, accept, comment):
		token = self.write_data().done_review(inv, accept, comment)
...
if __name__ == '__main__':
	...
	i = one.invite_review(two.entity.address, p ,'10000000000000000000')
	print(i)
